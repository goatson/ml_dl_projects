...
2. 정의
 ...
 - 톰미첼 : 어떤 작업 (Task:T)에 대한 컴퓨터 프로그램의 성능 P(Perfomance)로 측정하였을 때,
            경험 E(Experience)로 인해 성능이 향상되었다면, 이 컴퓨터 프로그램은 작업 T와 성능측정 P에 대해 경험 E로 학습한 것이다.
            (최초에는 허술하나 학습이 증가 될수록 우수한 성능을 발휘)
...
3. 데이터 과학(분석, 모델링 등) 절차
 ...
 2) 데이터 획득   ## 2월 14일까지 완료 ##
    - lv1 : 제공받는다 => 연구용 데이터, 경쟁(캐클, 챌린지), 공공 데이터, 정부기관요청(민원처리)
    ...
    - lv3 : Web Scraping (접근이 용이)
        => BS4, Request
    - lv4 : Crawling (ajax, 인증, 검색 등 사람의 손을 타야 하는 사이트)
        => Selenium
...
7. Library
    - numpy(날것의 도구)
        : Library for math, science
        : 고수준 수학
            - 선형대수학 (벡터, 행렬, 선형변환...)
            - 푸리에변환(시간의 함수, 연속적 신호, 주파수 ...)
        : 유사난수 생성
    - scipy(함수 알고리즘, 수학식 등 모두 구현)
        : 고성능 선형대수
        : 함수 최적화
        : 신호 처리
        : 특수 수학
